...
export default function ManageProducts() {
  ...
  const handleAddProduct = async (e) => {
    e.preventDefault();
    if (!newProduct.name || !newProduct.price || imageFiles.length < 1) {
          ...
          <div className="bg-white rounded-2xl shadow-sm border border-gray-100 p-6 md:p-8 sticky top-24">
            ...
            <form onSubmit={handleAddProduct} className="space-y-5">
              <div className="space-y-2">
                ...
                <input required type="text" placeholder="Product Name" value={newProduct.name} onChange={(e) => setNewProduct({ ...newProduct, name: e.target.value })} className="w-full bg-gray-50 border border-transparent rounded-xl p-3.5 outline-none focus:bg-white focus:border-[var(--color-primary)] transition-all" />
              ...
              <div className="space-y-2">
                ...
                <select value={newProduct.category} onChange={(e) => setNewProduct({ ...newProduct, category: e.target.value })} className="w-full bg-gray-50 border border-transparent rounded-xl p-3.5 outline-none focus:bg-white focus:border-[var(--color-primary)] transition-all appearance-none cursor-pointer">
                  ...
                  <label htmlFor="product-image" className="flex flex-col items-center justify-center gap-2 w-full min-h-[160px] border-2 border-dashed border-gray-100 rounded-2xl bg-gray-50 cursor-pointer hover:bg-gray-100 transition-all p-4 overflow-hidden relative">
                    {imagePreviews.length > 0 ? (
                      ...
                      <>
                        ...
                        <span className="text-xs font-bold text-gray-400 uppercase tracking-widest">Select up to 4 images</span>
                      </>
                    )}
                  </label>
                </div>
              ...
              <button type="submit" disabled={uploading} className="w-full py-4 rounded-xl bg-[var(--color-primary)] text-white font-bold shadow-lg shadow-[var(--color-primary)]/20 hover:scale-[1.02] active:scale-95 transition-all disabled:opacity-50 flex items-center justify-center gap-2">
                {uploading ? <><Loader2 className="animate-spin" size={20} /> Processing...</> : "Create Product"}
              </button>
            </form>
          </div>
        </div>
