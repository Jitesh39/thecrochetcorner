...
export default function HeroSettings() {
  ...
  const handleImageUpload = async (e) => {
    ...
    try {
      ...
      await setDoc(doc(db, "settings", "hero"), { images: newImages, typingLines }, { merge: true });
  ...
  const handleDeleteImage = async (index) => {
    ...
    try {
      await updateDoc(doc(db, "settings", "hero"), { images: newImages });
  ...
  const handleSaveText = async () => {
    setSaving(true);
    try {
      await setDoc(doc(db, "settings", "hero"), { images, typingLines }, { merge: true });
  ...
  return (
    <div className="space-y-8 animate-in slide-in-from-bottom duration-500">
      <div className="flex flex-col gap-1">
        ...
        <p className="text-sm text-gray-400 font-medium">Manage background images and animated text for the homepage hero.</p>
      </div>
        ...
        <div className="bg-white rounded-3xl shadow-sm border border-gray-100 p-8 flex flex-col h-full">
          <div className="flex items-center justify-between mb-8">
            <h2 className="text-lg font-bold text-gray-800 flex items-center gap-2">
              ...
              Background Images ({images.length}/5)
            </h2>
            <div className="relative">
              ...
              <label
                htmlFor="hero-upload"
                className={`flex items-center gap-2 px-4 py-2 rounded-xl font-bold text-xs uppercase tracking-widest transition-all cursor-pointer ${uploading || images.length >= 5
          ...
          <div className="grid grid-cols-2 sm:grid-cols-3 gap-4 flex-grow">
            ...
            {images.length === 0 && (
              <div className="col-span-full py-10 flex flex-col items-center justify-center text-gray-300 border-2 border-dashed border-gray-100 rounded-3xl bg-gray-50/50">
                ...
                <p className="text-xs font-bold uppercase tracking-widest mt-4">No images uploaded</p>
              </div>
            )}
          </div>
        </div>
